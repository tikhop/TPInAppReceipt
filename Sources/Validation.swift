...
public extension AppReceipt {
    ...
    func validate(
        ...
        guard let root = await finalAppleRootCertificateData(using: rootCertificate),
              let rootCert = try? Certificate(derEncoded: root.bytes) else {
...
fileprivate extension AppReceipt {
    func finalAppleRootCertificateData(using rootCertificate: Data? = nil) async -> Data? {
    ...
    func finalDeviceIdentifier(using deviceIdentifier: Data?) async -> Data? {
