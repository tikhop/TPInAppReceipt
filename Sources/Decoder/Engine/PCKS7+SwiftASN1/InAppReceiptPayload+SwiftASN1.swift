//
//  InAppReceiptPayload+SwiftASN1.swift
//  TPInAppReceipt
//
...
extension InAppReceiptPayload: BERImplicitlyTaggable {
    ...
    public init(berEncoded: SwiftASN1.ASN1Node, withIdentifier identifier: SwiftASN1.ASN1Identifier) throws {
        let attributes = try BER.set(of: InAppReceiptPayloadAttribute.self, identifier: identifier, rootNode: berEncoded)
    ...
    public func serialize(into coder: inout SwiftASN1.DER.Serializer, withIdentifier identifier: SwiftASN1.ASN1Identifier) throws {
        // Not implemented - receipt payload is read-only
    }

    public init(set: [InAppReceiptPayloadAttribute], rawBytes: ArraySlice<UInt8>) throws {
        ...
        for att in set {
            guard let field = AppReceiptField(rawValue: att.type) else {
                print(">>>> Missed type: \(att.type)")
                continue
            }
            ...
            case .inAppPurchaseReceipt:
                purchases.append(try InAppPurchase(berEncoded: att.value.bytes))
            ...
            case .unknown_6, .unknown_7, .unknown_13, .unknown_14, .unknown_20, .unknown_25:
                break
            }
        }
...
extension String {
    init(_ octetString: ASN1OctetString, as sourceEncoding: Encoding) throws {
        ...
        case Encoding.utf8:
            self = String(try ASN1UTF8String(derEncoded: octetString.bytes))
        case Encoding.ascii:
            self = String(try ASN1IA5String(derEncoded: octetString.bytes))
        default:
