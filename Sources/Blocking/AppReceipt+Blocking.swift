...
extension ReceiptValidator {
    ...
    ///
    /// Initializes a ``ReceiptValidator`` configured with all necessary verifiers
    ...
    public func validate_blocking(_ receipt: ReceiptValidatable) -> VerificationResult {
        for verifier in verifiers {
            if case .invalid(let error) = verifier.verify(receipt) {
...
fileprivate extension Bundle {
    static func appleRootCertificateData(testing: Bool) -> Data? {
...
public extension AppReceipt {
    ...
    /// This is useful for accessing the device's local receipt without making network requests.
    ///
    /// - Returns: An ``AppReceipt`` instance if the receipt exists and can be parsed, or nil if not available.
    /// - Throws: An error if the receipt cannot be parsed or decoded.
    static var local_blocking: AppReceipt? {
        get throws {
    ...
    ///
    /// Performs synchronous validation of this receipt instance. Automatically selects the appropriate
    /// root certificate based on the receipt's environment (production or test).
    ///
    /// - Parameters:
